...
import { Curso, Examen } from 'src/app/models';
...
export class AsignarExamenesComponent implements OnInit {
  ...
  mostrarColumnasAsignatura = ['id', 'nombre', 'asginatura', 'eliminar']
  ...
  private existe(id: number): boolean {
    let existe = false;
    console.info(`Cantidad examenes asignados: ${this.examenesAsignar.length}`);
  ...
  eliminarDelAsiganar(examen: Examen): void {
  ...
  asignar(): void {
    ...
    this.cursoService.asignarExamenes(this.curso, this.examenesAsignar)
      .subscribe(curso => {
        ...
        Swal.fire('Asingados', `Examens asignados con éxito. ${curso.nombre}`, 'success');
        this.tabIndex = 2;
      })
  }

  aliminarExamenDelCurso(examen: Examen): void {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.cursoService.eliminarExamen(this.curso, examen).subscribe(curso => {
          ...
          this.iniciarPaginador();
          Swal.fire('Eliminado', `Examen eliminao con exito`, 'success')
        });
      }
    });
